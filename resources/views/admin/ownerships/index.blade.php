...
@can('ownership_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.ownerships.create') }}">
                ...
                <tbody>
                    @foreach($ownerships as $key => $ownership)
                        <tr data-entry-id="{{ $ownership->id }}">
                            ...
                            <td>
                                @can('ownership_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.ownerships.show', $ownership->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('ownership_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.ownerships.edit', $ownership->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('ownership_delete')
                                    <form action="{{ route('admin.ownerships.destroy', $ownership->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('ownership_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.ownerships.massDestroy') }}",
