    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.carmodels.update", [$carmodel->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('manufacturer') ? 'is-invalid' : '' }}" name="manufacturer_id" id="manufacturer_id">
                    @foreach($manufacturers as $id => $manufacturer)
                        <option value="{{ $id }}" {{ (old('manufacturer_id') ? old('manufacturer_id') : $carmodel->manufacturer->id ?? '') == $id ? 'selected' : '' }}>{{ $manufacturer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('first_year') ? 'is-invalid' : '' }}" type="text" name="first_year" id="first_year" value="{{ old('first_year', $carmodel->first_year) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('last_year') ? 'is-invalid' : '' }}" type="text" name="last_year" id="last_year" value="{{ old('last_year', $carmodel->last_year) }}" required>
